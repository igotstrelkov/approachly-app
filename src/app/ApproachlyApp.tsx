...
const RANK_LADDER = [
  { name: "Rookie", lvl: 1, short: "RK", desc: "showing up for the first reps" },
  { name: "Bold", lvl: 5, short: "BD", desc: "walking over even when it's scary" },
  { name: "Fearless", lvl: 10, short: "FR", desc: "where approaching starts to feel normal" },
  { name: "Ironclad", lvl: 15, short: "IC", desc: "rejection barely registers anymore" },
  { name: "Legend", lvl: 20, short: "LG", desc: "you approach on pure instinct" },
] as const;
...
function monotonePath(pts: number[][]): string {
  const n = pts.length;
  if (n < 2) return "M" + pts.map((p) => p[0].toFixed(1) + " " + p[1].toFixed(1)).join(" L");
...
function buildChart(trend: number[], W: number, H: number) {
  ...
  const refLine = "M" + first[0].toFixed(1) + " " + first[1].toFixed(1) + " L" + last[0].toFixed(1) + " " + last[1].toFixed(1);

  const delta = trend[0] - trend[trend.length - 1]; // positive = improvement (lower fear is better)
  const hasDelta = trend.length >= 2;
  let chartArrow: string, chartDelta: string, chartTrendColor: string, chartTrendTint: string;
  if (!hasDelta || Math.abs(delta) <= 0.05) {
    chartArrow = "—"; chartDelta = "no change yet"; chartTrendColor = "var(--ash)"; chartTrendTint = "var(--slate)";
  } else if (delta > 0.05) {
    chartArrow = "▼"; chartDelta = Math.abs(delta).toFixed(1) + " since start"; chartTrendColor = "var(--go)"; chartTrendTint = "rgba(52,209,126,.12)";
  } else {
    chartArrow = "▲"; chartDelta = Math.abs(delta).toFixed(1) + " since start"; chartTrendColor = "var(--amber)"; chartTrendTint = "rgba(224,160,48,.12)";
  }
  return {
    chartLine: line, chartArea: area, chartRefLine: refLine, chartW: W, chartH: H,
    chartDotX: last[0].toFixed(1), chartDotY: last[1].toFixed(1),
    chartCurrent: trend[trend.length - 1].toFixed(1),
    chartArrow, chartDelta, chartTrendColor, chartTrendTint,
...
type Screen = "home" | "hype" | "log" | "reward" | "you" | "quiz" | "ranks";
...
export default function ApproachlyApp({
  ...
  const nextRankHint = (() => {
    const thresholds: [number, string][] = [[5, "Bold"], [10, "Fearless"], [15, "Ironclad"], [20, "Legend"]];
    ...
    const nextTierLevel = PRESTIGE_START + (Math.floor((level - PRESTIGE_START) / PRESTIGE_BAND) + 1) * PRESTIGE_BAND;
  ...
  const fearLabel =
    ...
    : trend[0] - trend[trend.length - 1] > 0.05 ? "Your fear, falling"
    : trend[0] - trend[trend.length - 1] < -0.05 ? "Your fear, lately"
    : "Your fear, so far";
  ...
  const journeyRanks = RANK_LADDER.map((r) => {
    ...
    return {
      ...r, reached, current, isNext, distance,
      subtitle: current || isNext ? r.desc : `Level ${r.lvl}+`,
      mark: current ? "● Current" : reached ? "✓" : `${distance} ${distance === 1 ? "level" : "levels"} away`,
      nodeBg: current ? "var(--ember)" : reached ? "rgba(255,178,62,.2)" : "var(--charcoal)",
      nodeColor: current ? "#1a0f08" : reached ? "var(--ember)" : "var(--ashDim)",
      nodeBorder: current ? "2px solid var(--ember)" : reached ? "1px solid rgba(255,178,62,.4)" : "1px solid var(--slate)",
      cardBg: current ? "rgba(255,178,62,.12)" : "var(--charcoal)",
      cardBorder: current ? "var(--ember)" : isNext ? "var(--slateHi)" : "var(--slate)",
      nameColor: reached || current ? "var(--bone)" : "var(--ashDim)",
      markColor: current ? "var(--ember)" : reached ? "var(--go)" : isNext ? "var(--ember)" : "var(--ashDim)",
    };
  });
              ...
              <>
                ...
                <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 18 }}>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 9 }}>
                    ...
                    <span style={{ fontFamily: MONO, fontSize: 13, color: "var(--ash)" }}>/10</span>
                  </div>
                ...
                <svg viewBox={`0 0 ${chart.chartW} ${chart.chartH}`} width="100%" style={{ display: "block", overflow: "visible", marginBottom: 6 }}>
                  ...
                  <path d={chart.chartRefLine} fill="none" stroke="var(--ash)" strokeWidth="1.5" strokeDasharray="3 6" strokeLinecap="round" opacity="0.3" style={{ animation: "aFadeIn 1s ease .6s both" }} />
            ...
            {!isFresh && (
              <div style={{ marginTop: 46 }}>
                ...
                <div>
                  ...
                  <div style={{ padding: "17px 0", borderBottom: "1px solid var(--slate)" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                      <span style={{ fontSize: 14.5, color: "var(--bone)" }}>This week</span>
                      {user.repsThisWeek >= user.weeklyGoal ? (
                        <span style={{ fontFamily: MONO, fontSize: 14, color: "var(--go)", fontWeight: 700 }}>Goal hit ✓</span>
                      ) : (
                        <span style={{ fontFamily: MONO, fontSize: 14, color: "var(--bone)" }}><span style={{ color: "var(--go)" }}>{user.repsThisWeek}</span> / {user.weeklyGoal} · goal</span>
                      )}
                    </div>
                    <div style={{ display: "flex", gap: 5 }}>
                      {Array.from({ length: Math.max(1, user.weeklyGoal) }).map((_, i) => (
                        <div key={i} style={{ flex: 1, height: 6, borderRadius: 999, background: i < Math.min(user.repsThisWeek, user.weeklyGoal) ? "var(--go)" : "var(--slate)" }} />
                      ))}
                    </div>
                  </div>
                  {[
                  ...
                  <button onClick={() => nav("ranks")} style={{ width: "100%", textAlign: "left", cursor: "pointer", display: "block", marginTop: 18, background: "var(--charcoal)", border: "1px solid var(--slate)", borderRadius: 16, padding: 16 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
                      <div style={{ width: 46, height: 46, flexShrink: 0, borderRadius: 13, background: "linear-gradient(135deg,var(--ember),var(--flare))", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: DISPLAY, fontSize: 22, color: "#1a0f08" }}>{level}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 15, color: "var(--bone)" }}><span style={{ fontWeight: 700 }}>{rank}</span> <span style={{ color: "var(--ash)" }}>· Level {level}</span></div>
                        <div style={{ fontSize: 12.5, color: "var(--ash)", marginTop: 2 }}>{nextRankHint}</div>
                      </div>
                      <span style={{ color: "var(--ash)", fontSize: 20 }}>›</span>
                    </div>
                    <div style={{ height: 6, borderRadius: 999, background: "var(--slate)", overflow: "hidden", marginTop: 14 }}>
                      <div style={{ height: "100%", width: `${levelPct}%`, background: "linear-gradient(90deg,var(--ember),#FFCF7A)", borderRadius: 999 }} />
                    </div>
                    <div style={{ fontFamily: MONO, fontSize: 11.5, color: "var(--ash)", marginTop: 8 }}>{xpToNext} XP to Level {level + 1} · View journey ›</div>
                  </button>
                </div>
              </div>
            )}
        ...
        {screen === "ranks" && (
          <div style={{ minHeight: "100vh", padding: "calc(env(safe-area-inset-top, 0px) + 20px) 22px 40px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 18 }}>
              <div style={{ width: 66, height: 66, flexShrink: 0, borderRadius: 20, background: "linear-gradient(135deg,var(--ember),var(--flare))", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: DISPLAY, fontSize: 30, color: "#1a0f08" }}>{level}</div>
              <div>
                <div style={{ fontFamily: DISPLAY, fontSize: 26, textTransform: "uppercase", color: "var(--bone)", lineHeight: 1 }}>Level {level}</div>
                <div style={{ fontFamily: MONO, fontSize: 12, color: "var(--ash)", marginTop: 3 }}>{into} / {need} XP</div>
              </div>
            </div>
            <div style={{ height: 9, borderRadius: 999, background: "var(--slate)", overflow: "hidden", marginBottom: 8 }}>
              <div style={{ height: "100%", width: `${levelPct}%`, background: "linear-gradient(90deg,var(--ember),#FFCF7A)", borderRadius: 999 }} />
            </div>
            <div style={{ fontSize: 12.5, color: "var(--ash)", marginBottom: 6 }}>{xpToNext} XP to Level {level + 1}</div>
            <div style={{ fontSize: 12, color: "var(--ash)", marginBottom: 32 }}>XP is courage banked — every rep counts, more the scarier it felt.</div>

            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 16 }}>
              <span style={{ ...eyebrow("var(--ash)"), letterSpacing: 2 }}>Ranks · permanent climb</span>
              <span style={{ fontFamily: MONO, fontSize: 11, color: "var(--ash)" }}>{user.totalApproaches} approaches</span>
            </div>

            <div style={{ display: "flex", flexDirection: "column" }}>
              {journeyClimb.map((r, i, arr) => {
                const isTop = i === 0, isBottom = i === arr.length - 1;
                ...
                const seg = (filled: boolean): CSSProperties =>
                  filled
                    ? { background: "linear-gradient(180deg, var(--ember), var(--go))" }
                    : { backgroundImage: "repeating-linear-gradient(var(--ashDim) 0 3px, transparent 3px 8px)" };
                return (
                  <div key={r.name} style={{ display: "flex", gap: 14, alignItems: "stretch", minHeight: 84 }}>
                    <div style={{ width: 34, flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center" }}>
                      <div style={{ width: 2, flex: 1, ...(isTop ? { background: "transparent" } : seg(topFilled)) }} />
                      <div style={{ width: 34, height: 34, flexShrink: 0, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: MONO, fontSize: 11, fontWeight: 700, background: r.nodeBg, color: r.nodeColor, border: r.nodeBorder, zIndex: 1 }}>{r.short}</div>
                      <div style={{ width: 2, flex: 1, ...(isBottom ? { background: "transparent" } : seg(botFilled)) }} />
                    </div>
                    <div style={{ flex: 1, alignSelf: "center", display: "flex", alignItems: "center", gap: 13, padding: "13px 15px", borderRadius: 14, background: r.cardBg, border: `1px solid ${r.cardBorder}` }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 700, fontSize: 15, color: r.nameColor }}>{r.name}</div>
                        <div style={{ fontSize: 12, color: "var(--ash)", marginTop: 1 }}>{r.subtitle}</div>
                      </div>
                      <span style={{ fontFamily: MONO, fontSize: 11, color: r.markColor, whiteSpace: "nowrap" }}>{r.mark}</span>
                    </div>
                  </div>
                );
              })}
